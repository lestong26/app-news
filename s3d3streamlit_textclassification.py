...
# 8: Adding plots for filtered data
...
# Text input for keywords used to filter the data
    keywords = st.text_input(
        label='Keywords for filtering the data. If multiple keywords, make a comma-separated list',
        value=''
    )

    # Split the keywords by comma and strip any leading/trailing whitespace
    keywords = [kw.strip() for kw in keywords.split(',')]

    # Display the keywords entered
    st.write(keywords)

    # Multiselect widget to choose columns in which to search for keywords
    search_cols = st.multiselect(
        'Select columns where keywords will be searched',
        df.columns)

    # If any columns are selected for searching
    if search_cols:
        marker_cols = list()
    
        # For each selected column, create a new marker column indicating presence of keywords
        for col in search_cols:
            df[col+'_marker'] = df[col].str.contains('|'.join(keywords), case=False)
            marker_cols.append(col+'_marker')

        # List of marker columns
        search_markers = [x + '_marker' for x in search_cols]
    
        # Create a 'marker' column to indicate if any of the markers are True (keyword found)
        df['marker'] = df[search_markers].sum(axis=1)
        df['marker'] = df['marker'] > 0

        # Filter the DataFrame to only include rows where the 'marker' is True
        df = df[df['marker']]
    
    # Toggle to show a preview of the filtered dataset
